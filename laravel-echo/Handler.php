...
class Handler extends WebsocketHandler
{
    ...
    public function handleWebsocket(
        WebsocketEvent $event,
        Context $context
    ): HttpResponse {
        try {
            $method = Str::camel(
                "handle_" . Str::lower($event->getEventType() ?? "")
            );

            if (!method_exists($this, $method)) {
                throw new \InvalidArgumentException(
                    "Event type {$event->getEventType()} has no handler implemented."
                );
            }
    ...
    protected function handleDisconnect(
        ...
        $this->subscriptionRepository->clearConnection(
    ...
    protected function handleMessage(
        ...
        if (!isset($eventBody["event"])) {
            throw new \InvalidArgumentException(
                "event missing or no valid json"
            );
        }
    ...
    protected function subscribe(WebsocketEvent $event, Context $context): void
    {
        ...
        $this->subscriptionRepository->subscribeToChannel(
            $event->getConnectionId(),
            $channel
        );
    ...
    protected function unsubscribe(
        ...
        $this->subscriptionRepository->unsubscribeFromChannel(
            $event->getConnectionId(),
            $channel
        );
    ...
    public function broadcastToChannel(
        WebsocketEvent $event,
        Context $context
    ): void {
        ...
        $this->subscriptionRepository
            ...
            ->each(
                fn(string $connectionId) => $this->sendMessageToConnection(
                    $connectionId,
                    $data
                )
            );
    ...
    public function sendMessage(
        ...
        $this->connectionRepository->sendMessage(
            $event->getConnectionId(),
